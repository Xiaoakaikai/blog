      ...
      <el-tab-pane label="系统配置" name="one">
        ...
        <el-form style="margin-left: 20px;" label-position="left" label-width="140px">
          ...
          <el-form-item label="启用邮箱发送功能">
            <el-radio v-for="item in yesNoDictList" :key="parseInt(item.value)" v-model="form.openEmail" :label="parseInt(item.value)"
          ...
          <el-form-item label="注册用户邮件激活">
            <el-radio v-for="item in openDictList" :key="item.value" v-model="form.openEmailActivate"
                      :label="item.value" border size="medium">{{ item.label }}
            </el-radio>
          </el-form-item>
  ...
  <el-tab-pane label="本地文件存储" name="tow">
      ...
      <el-tab-pane label="仪表盘通知" name="five">
        ...
        <div class="editor-container">
          <mavon-editor placeholder="开始编辑...." :subfield="false" style="height: 500px" ref=md v-model="form.dashboardNotificationMd" @imgDel="" @change=""
                        @imgAdd=""/>
        </div>
