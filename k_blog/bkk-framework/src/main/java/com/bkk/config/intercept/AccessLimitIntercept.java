package com.bkk.config.intercept;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        try {
            ...
            if (handler instanceof HandlerMethod) {
                ...
                String key = IpUtils.getIpAddress(request) + request.getRequestURI();
                // 获取redis的value
                ...
                if (maxTimes == null) {
                    ...
                    redisCache.setCacheObject(key, 1, time, TimeUnit.SECONDS);
                } else if (maxTimes < count) {
                    // 如果redis中的时间比注解上的时间小则表示可以允许访问,这是修改redis的value时间
                    redisCache.setCacheObject(key, maxTimes + 1, time, TimeUnit.SECONDS);
                } else {
                    // 请求过于频繁
